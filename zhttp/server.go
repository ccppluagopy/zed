...
func NewServer(addr string, handler func(http.ResponseWriter, *http.Request)) *Server {
	ts := NewUnstartedServer(addr, http.HandlerFunc(handler))
	ts.Start()
	return ts
}
...
//
// The caller should call Close when finished, to shut it down.
func NewUnstartedServer(addr string, handler func(http.ResponseWriter, *http.Request)) *Server {
	return &Server{
...
// The caller should call Close when finished, to shut it down.
func NewTLSServer(addr string, handler func(http.ResponseWriter, *http.Request)) *Server {
	ts := NewUnstartedServer(addr, http.HandlerFunc(handler))
	//ts.StartTLS()
	return ts
}
